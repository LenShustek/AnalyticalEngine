...
#define MIN_uSTEP_TIME_USEC 250  // minimum time between microsteps for reliable operation
...
struct switchnum_t switch_numbers[] = {  //map from switch number to switch name
   { SW_A2, "A2" },
   { SW_F2, "F2" },
   { SW_CTR2, "CTR2" },
   { SW_CTR1, "CTR1" },
...
void setmotor(enum motornum_t motornum /*0 to NUM_MOTORS-1*/, int boardnum /*1 to 6*/, int motorposn /*1 to 16*/) {
   ...
   assert(pmd, "bad motor in setmotor: number %d, board %d, position %d", motornum, boardnum, motorposn);
   assert(!pmd->assigned, "motor already assigned: %s", pmd->axle_name);
...
*/
struct motord_t motor_descriptors[] = {  // an unordered list of descriptors for defined motors
   //  put longer names first so they get scanned first in case later ones are prefixes
   //  default gears are none (1:1)
   { FP2K_R, ROTATE, "fp2k", "fixed long pinion 2 lock", GEARMOTOR_BIG, GEARMOTOR_SMALL },
   { MP2K_R, ROTATE, "mp2k", "movable long pinion 2 lock", GEARMOTOR_BIG, GEARMOTOR_SMALL },
   ...
   { A2_R, ROTATE, "a2r", "A2 finger rotate", MILL_DIGIT_GEAR_BIG * GEARMOTOR_BIG, MILL_DIGIT_GEAR_SMALL * GEARMOTOR_SMALL, A2_L /*compensating lifter*/ },
   { F2_L, LIFT, "f2l", "carriage 2 finger lift" },
   { F2_R, ROTATE, "f2r", "carriage 2 finger rotate", MILL_DIGIT_GEAR_BIG * GEARMOTOR_BIG, MILL_DIGIT_GEAR_SMALL * GEARMOTOR_SMALL, F2_L /*compensating lifter*/ },
   ...
   { CW2_L, LIFT, "cw2l", "carry warning 2 lift" },
   ...
   { S2_L, LIFT, "s2l", "store stack 2 lift" },
   { S2_R, ROTATE, "s2r", "store stack 2 rotate", STORE_DIGIT_GEAR_BIG * GEARMOTOR_BIG, STORE_DIGIT_GEAR_SMALL * GEARMOTOR_SMALL, S2_L /*compensating lifter*/ }, // should remove gearmotor!
...
void assign_motors(void) {  // assign the physical location of the driver for each motor
   ...
   setmotor(SIGN_L, 1, 9);   // sign lift
   setmotor(SIGN_R, 1, 10);  // sign rotate
   ...
   setmotor(CTR1_L, 2, 8);   // counter 1 lift
   setmotor(CTR1_R, 2, 9);   // counter 1 rotate
   setmotor(CSK2_R, 2, 10);  // carry sector keepers rotate
   setmotor(S2_L, 2, 11);    // store 2 lift
   setmotor(S2_R, 2, 12);    // store 2 rotate
   setmotor(CTR2_L, 2, 13);  // counter 2 lift
   setmotor(CTR2_R, 2, 14);  // counter 2 rotate
   setmotor(CSK2_L, 2, 15);  // carry sector keepers lift
   setmotor(CW2_R, 2, 16); } // carry warning arms rotate
   ...
   for (struct motord_t* pmd = motor_descriptors; pmd->motor_number != -1; ++pmd) {
      ...
      if (motornum != NM) {
         ...
         if (pmd->gear_big == 0) {  // if the gear ratio is not already set
            pmd->gear_big = 1;
            pmd->gear_small = 1; }                    // assume 1:1
         pmd->keep_on = true;  // default is to keep motors on once powered on
         static int off_motor[] = { RK_L, -1 };
         //*** special cases we need to configure
         for (int* p = off_motor; *p != -1; ++p)
            if (motornum == *p) {
               pmd->full_steps = true;  // motors rounded to full steps to allow powering off between movements
               pmd->keep_on = false; }
         if (motornum == RR_R || motornum == S1_R) // store and rack restore wheels have terrible rotational backlash
            pmd->backlash_degrees = 2; // number of degrees of extra rotation
      } }
...
void power_motor(struct motord_t* pmd, enum motor_state_t onoff, bool forceoff) {  // power a motor on or off
   ...
   assert(pmd && pmd->assigned, "unassigned motor in power_motor: %s", pmd ? pmd->axle_name : "");
   //if (pmd->motor_number == S1_L || pmd->motor_number == RP2_L) //TEMP
   //    Serial.printf("turning motor %s %s, is %s\n", pmd->axle_name, onoff==OFF ? "OFF":"ON", pmd->motor_state= OFF ? "OFF":"ON"); //TEMP
   if (pmd->motor_state != onoff) {  // current state is different
      int motornum = pmd->motor_number;
      if (onoff == OFF) {                                                                 // check conditions for denying power off
         if (!forceoff && (pmd->keep_on || pmd->temp_on || pmd->microstep_offset != 0)) {  // can't turn off a motor between full-step positions
            if (debug >= 4 && !pmd->keep_on && pmd->microstep_offset != 0) Serial.printf("  motor %d (%s on board %d position %d) not at full step so left on\n",
                     motornum, pmd->axle_name, pmd->board_number, pmd->board_position);
            return; }
         if (abs(pmd->current_position) == abs(TIGHTEN_LOCK_DEGREES))  // HACK
            pmd->current_position = 0; }                                                            // recognize that turning off preloaded locks releases them
      else                                                          /*ON*/
         pmd->microstep_offset = 0;                                  // microstep offset goes back to zero when powering on
...
void power_motors(enum motor_state_t onoff) {  // power all motors on or off
   if (debug >= 5) Serial.printf("powering motors %s\n", onoff == ON ? "on" : "off");
   ...
   for (struct motord_t* pmd = motor_descriptors; pmd->motor_number != -1; ++pmd)  // all defined motors
      if (pmd->assigned)                                                            // we have assigned its position
         power_motor(pmd, onoff, true); } // force on or off
...
bool check_abort(void) {  // check for conditions that abort the current movements
   if (Serial.available()) {
      ...
      if (chr == ESC) {  // (2) ESC from the keyboard
         ...
         return true; }
      if (chr == '/')  // (not an abort: '/' stops "repeat")
         stop_repeat = true; }
...
void do_test(void) { //specialtest
   Serial.println("enter chars, . to exit");
   while (1)
      if (Serial.available()) {
         char chr = Serial.read();
         if (chr == '.') break;
...
void do_testmotor(const char** pptr) {
   struct motord_t* pmd = scan_axlename(pptr, ANY_MOVEMENT, false); // motor name or board/posn numbers
   if (pmd) {
      Serial.printf("testing %s (%s) on board %d at position %d\n",
        pmd->axle_name, pmd->axle_descr, pmd->board_number, pmd->board_position);
      got_error = false;
      pmd->temp_on = true;
      while (!got_error) {
        #define TESTMOTOR_DEGREES 45
        #define TESTMOTOR_TIME 250
          queue_movement(pmd, ROTATE, TESTMOTOR_DEGREES);
          do_movements(TESTMOTOR_TIME*1000);
          delay(TESTMOTOR_TIME);
          queue_movement(pmd, ROTATE, -TESTMOTOR_DEGREES);
          do_movements(TESTMOTOR_TIME*1000); 
          delay(TESTMOTOR_TIME);}
      pmd->temp_on = false;
      power_motor(pmd, OFF, true); // force off
      return; } 
   else error("bad board or motor number"); }
...
bool do_movements(unsigned long duration_usec) {
   ...
   if (debug >= 3) {
      ...
      for (struct motord_t* pmd = motor_descriptors; pmd->motor_number != -1; ++pmd)
         if (pmd->move_queued) {
            unsigned usteps_to_be_done = pmd->end_pct <= 99 ? pmd->usteps_needed : pmd->usteps_needed * 100 / pmd->end_pct;  // if the movement extends beyond this time unit, adjust steps needed to just this time unit
            Serial.printf(" %s(%d)", pmd->axle_name, usteps_to_be_done);
            unsigned time_needed = usteps_to_be_done * MIN_uSTEP_TIME_USEC;  // minimum time needed for this motor to do its movement
            if (time_needed > max_time) max_time = time_needed; }
      Serial.printf(" in %d msec\n", (max(max_time, duration_usec) + 500) / 1000); }
   if (check_abort()) {
            ...
            if (timenow > pmd->start_time) {                                  // we're past the start time for this motor
               ...
               if (deltatime > MIN_uSTEP_TIME_USEC && deltatime >= pmd->step_delta_time) {
                  ...
                  if (++pmd->usteps_done >= pmd->ending_ustep) {  // if this motor is done for this time unit
                     if (!pmd->keep_on) power_motor(pmd, OFF);     // power off if we're allowed to
...
void queue_movement(  // queue an elemental movement to happen during this time unit
   ...
   assert(pmd, "bad call to queue_movement");
   if (pmd->move_queued) {
      Serial.printf("ERROR: axle %s is already scheduled to move\n", pmd->axle_name);
      return; }
   ...
   */
   ...
   if (movetype == ROTATE) {                                                       // ROTATE: distance is signed degrees
      if (pmd->backlash_degrees                                                     // if there is backlash compensation
            && pmd->previous_distance                                                 // and there was a previous movement
            && distance * pmd->previous_distance < 0) {                               // and the direction is going to change (signs are different)
         distance += distance > 0 ? pmd->backlash_degrees : -pmd->backlash_degrees;  // add or subtract the backlash
         if (debug >= 3) Serial.printf("  ...motor %s compensated for backlash to %d degrees\n", pmd->axle_name, distance); }
      pmd->previous_distance = distance;
      numer = distance * pmd->gear_big * uSTEPS_PER_ROTATION;
      denom = 360 * pmd->gear_small;
      pmd->usteps_needed = numer / denom;
      if (!pmd->full_steps) {             // could be a fraction of a microstep
   ...
   pmd->start_pct = start;
   pmd->end_pct = end;
   if (movetype == ROTATE && pmd->compensating_lifter)  // this rotate needs a compensating counter-rotation by same # degrees of the associated lifter
      queue_movement(motor_num_to_descr[pmd->compensating_lifter], ROTATE, -distance, start, end); }
...
struct motord_t* move_to_zero(struct fct_move_t /*lifter*/* move, bool calibrate) {
   // Rotate the digit wheel (F,A,S,RR) whose lifter is given
   // until it gets to the zero point, or the switch point if we're  calibrating.
   // Return the axle being rotated.
   static struct {  // map from lifter to rotator and switch
      int lift_motor;
      int rotate_motor;
      int switch_number; } switchmap[] = {
      { F2_L, F2_R, SW_F2 }, { F3_L, F3_R, SW_F3 }, { A1_L, A1_R, SW_A1 }, { A2_L, A2_R, SW_A2 }, { A3_L, A3_R, SW_A3 },
      { S1_L, S1_R, SW_S1 }, { S2_L, S2_R, SW_S2 }, { S3_L, S3_R, SW_S3 }, { S4_L, S4_R, SW_S4 }, { S5_L, S5_R, SW_S5 }, { S6_L, S6_R, SW_S6 },
      { RR_L, RR_R, SW_RR }, { SIGN_L, SIGN_R, SW_SIGN }, { -1 } };
   int rotate_motor = -1, switch_number = 0, limit, calibration_degrees = 0, pre_movement_degrees;
   for (int ndx = 0; switchmap[ndx].lift_motor >= 0; ++ndx)  // find the right map
      if (switchmap[ndx].lift_motor == move->motor_num) {
         ...
         break; }
   assert(rotate_motor >= 0, "bad lifter in move_to_zero");
   struct motord_t* rotate_axle = motor_num_to_descr[rotate_motor];
   if (!calibrate) {
      calibration_degrees = config.finger_zero_degrees[rotate_axle->motor_number].degrees;
      if (calibration_degrees == -1) {  
        error("axle not calibrated", rotate_axle->axle_name);
         goto abort; }}
   // We must rotate at least 360 degrees in this process to make sure the finger is against the nib.
   // We don't know how many degrees it will take to rotate to the switch, so we just make sure to do
   // 360 between the pre-movmement before moving to the switch, and the post-switch calibrated movement.
   pre_movement_degrees = 360 - calibration_degrees; 
   if (debug >= 1) Serial.printf("rotating %s %d degrees to engage finger\n", rotate_axle->axle_name, pre_movement_degrees);
   ...
   queue_movement(rotate_axle, ROTATE, pre_movement_degrees);  // rotate 10 digits to ensure the wheel engages with the finger
   if (!do_movements(timeunit_degree_usec * pre_movement_degrees)) goto abort;
   limit = 370;
   while (--limit && read_switch(switch_number) == 0) {  // if it's sitting on the switch
      if (script_step && !do_step_wait()) goto abort;
   ...
   if (!limit) {
      ...
      goto abort; }
   if (!calibrate) {
      if (debug >= 1) Serial.printf("rotating %s %d degrees to zero\n", rotate_axle->axle_name, calibration_degrees);
      if (script_step && !do_step_wait()) goto abort;
      queue_movement(rotate_axle, ROTATE, calibration_degrees);  // now make the final adjustment to zero
      do_movements(timeunit_degree_usec * calibration_degrees);  }
   return rotate_axle;
abort:
...
void do_zero(const char** pptr) {  // zero {Fn|An|Sn|RR} [calibrate]
   ...
   int chr;
   lift_move = do_function(fct_zero, pptr);  // parse the wheel (F,An,Sn,RR, SIGN) and get the finger lifter; no moves yet
   if (!lift_move) return; 
   bool calibrate = scan_key(pptr, "calibrate");  // are we doing calibration?
   rotate_axle = move_to_zero(lift_move, calibrate);  // move to zero, or to the switch if calibrating
...
void do_calibrate(const char** pptr) {  // store a calibration value, as in "calibrate s1r 47"
   ...
   if ((pmd = scan_axlename(pptr, ROTATE, true))) {
      ...
      if (scan_int(pptr, &degrees, -360, +360)) {
         ...
         *target = degrees;  //make the change
         write_config(); }
      else Serial.printf("axle %s calibration is %d degrees past the switch\n",
                            pmd->axle_name, config.finger_zero_degrees[pmd->motor_number].degrees); } }
//*
